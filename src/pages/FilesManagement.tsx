...
const FilesManagement = () => {
    ...
    const onUploadFiles = async () => {
        ...
        if(filterRes.length){
            const totalFiles= files.data.length + filterRes.length;
            //files total update
            try{
               await apiService.updateProjectDataCount(id!,{filesCount: totalFiles});
               setTimeout(()=>{
                setFiles((prev)=>({...prev,
                    data:[...filterRes, ...prev.data],
                    inpFiles: prev.inpFiles.filter((f)=>!filterRes.some((r)=>r.name === f.file.name)) //r.id, f.id map for smae naem?
                }));
                filterRes.forEach((f)=>{
    ...
    const handleDelFile = async (fileId: string|number) => {
        try {
            await apiService.delFile(fileId);
            ...
            setFiles((prev) => ({
                ...prev,
                data: prev.data.filter((f) => f.id !== fileId)
            }));
        } catch (err) {
